/**
 * products-tutorial.js
 * - 상품 페이지 튜토리얼(온보딩 워크스루) 기능
 */

const PRODUCTS_TUTORIAL_STORAGE_KEY = 'growth_launchpad_products_tutorial_completed';

// 튜토리얼 단계 정의
const productsTutorialSteps = [
    {
        id: 'intro',
        title: '상품 관리',
        description: '여기에서 상품의 정보를 확인하고 고객사에게 빠르게 대응하세요.',
        target: null,
        position: 'center'
    },
    {
        ...
        target: '#btn-add-product',
        position: 'left'
    },
    {
        id: 'product-card',
        title: '상품 정보',
        description: '각 상품의 상세 정보와 견적서 등록까지 한 곳에서 관리해보세요.',
        target: '#product-list',
        position: 'top'
    },
    {
        id: 'finish',
        title: '이제 직접 시작해보세요!',
        description: '튜토리얼을 완료했습니다. 상품을 자유롭게 관리해보세요!',
        target: null,
        position: 'center'
    }
];
...
function isTutorialCompleted() {
    return localStorage.getItem(PRODUCTS_TUTORIAL_STORAGE_KEY) === 'true';
}

// 튜토리얼 완료 표시
function markTutorialCompleted() {
    localStorage.setItem(PRODUCTS_TUTORIAL_STORAGE_KEY, 'true');
}
...
function nextStep() {
    ...
    if (currentStep >= productsTutorialSteps.length) {
        finishTutorial();
    } else {
        showStep(currentStep);
    }
...
function showStep(stepIndex) {
    const step = productsTutorialSteps[stepIndex];
    if (!step) return;
    ...
    if (totalEl) totalEl.textContent = productsTutorialSteps.length;
    ...
    if (nextBtn) {
        if (stepIndex === 0) {
            nextBtn.textContent = '지금부터 빠르게 둘러보세요';
        } else if (stepIndex === productsTutorialSteps.length - 1) {
...
function positionTooltip(element, position) {
    ...
    // 상품 등록 버튼의 경우 화면 크기에 따라 자동 조정
    if (element.id === 'btn-add-product') {
        // 모바일에서는 위쪽에 표시
        if (viewportWidth <= 768) {
            ...
            // 화면 밖으로 나가지 않도록 조정
            if (parseFloat(tutorialTooltip.style.left) < 10) {
                tutorialTooltip.style.left = '10px';
            }
            if (parseFloat(tutorialTooltip.style.left) + tooltipRect.width > viewportWidth - 10) {
                tutorialTooltip.style.left = `${viewportWidth - tooltipRect.width - 10}px`;
            }
        } else {
            // 데스크톱에서는 왼쪽에 표시
            ...
            // 화면 밖으로 나가지 않으면 위쪽에 표시
            if (parseFloat(tutorialTooltip.style.left) < 10) {
                tutorialTooltip.classList.remove('tooltip-left');
                tutorialTooltip.classList.add('tooltip-top');
                tutorialTooltip.style.top = `${rect.top - tooltipRect.height - 20}px`;
                tutorialTooltip.style.left = `${rect.left + rect.width / 2 - tooltipRect.width / 2}px`;
            }
    ...
    // 화면 밖으로 나가지 않도록 조정
    const tooltipTop = parseFloat(tutorialTooltip.style.top);
    const tooltipLeft = parseFloat(tutorialTooltip.style.left);
    
    if (tooltipTop < 10) {
        tutorialTooltip.style.top = '10px';
    }
    if (tooltipTop + tooltipRect.height > viewportHeight - 10) {
        tutorialTooltip.style.top = `${viewportHeight - tooltipRect.height - 10}px`;
    }
    if (tooltipLeft < 10) {
        tutorialTooltip.style.left = '10px';
    }
...
// 튜토리얼 재시작 (디버깅용)
function restartProductsTutorial() {
    localStorage.removeItem(PRODUCTS_TUTORIAL_STORAGE_KEY);
    currentStep = 0;
    startTutorial();
}

// 전역 함수로 노출
window.restartProductsTutorial = restartProductsTutorial;

// 페이지 로드 시 초기화
document.addEventListener('DOMContentLoaded', function() {
    // products.html 페이지에서만 실행
    if (window.location.pathname.includes('products.html')) {
        initProductsTutorial();
    }
});
